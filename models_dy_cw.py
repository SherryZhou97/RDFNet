...
class attention(nn.Module):
    def __init__(self):
        ...
        self.avgpool = nn.AvgPool2d(5,1,2)
...
class FF_block(nn.Module):
    def __init__(self):
        ...
        self.shrinkage = Shrinkage_CW((1, 1), 64)
    ...
    def forward(self, x):
        ...
        x = self.conv12(x)
        # softFrk = torch.sign(x) * F.relu(torch.abs(x)-self.softThr)
        softFrk = self.shrinkage(x)
        Fx = F.relu(self.conv13(softFrk))
        Fx = self.conv14(Fx)
...
class Dynamic_block(nn.Module):
    ...
    def forward(self, x):
        ...
        block1, sym1 = self.block(x)
        block2, sym2 = self.block(x)
        block3, sym3 = self.block(x)
...
if __name__ == '__main__':
    ...
    Phi = sio.loadmat('/home1/users/dinglihe/Transformer_CASSI/Data/mask.mat')['mask']
