...
from models_dy_cw import Twist_net
from utils import *
...
from mask_guided import mask_meas
...
model = Twist_net(nFrame=28, nPhase=5).cuda()
...
if last_train != 0:
    model = torch.load('model_save_filename/model_epoch_{}.pth'.format(last_train))
...
def train(epoch, learning_rate, logger):
    ...
    for i in range(batch_num):
        ...
        # Xinput, Yinput = mask.forward(gt, mask3d_batch)
        optimizer.zero_grad()
        # print(Xinput.shape, gt.shape, mask3d_batch.shape, Yinput.shape)
        Recon_img, Loss = model(Xinput, gt, mask3d_batch, mask3d_batch, Yinput)
        epoch_loss += Loss.data
        Loss.backward()
        optimizer.step()
    end = time.time()
    logger.info("===> Epoch {} Complete: Avg. Loss: {:.6f} time: {:.2f}".format(epoch, (epoch_loss/batch_num).item(),  (end - begin)))
...
def test(epoch, logger):
    ...
    logger.info('===> Epoch {}: testing psnr = {:.2f}, ssim = {:.3f}, time: {:.2f}'.format(epoch, all_psnr_mean, all_ssim_mean, (end - begin)))
    model.train()      
...
def main(learning_rate):
    ...
    for epoch in range(last_train + 1, last_train + max_epoch + 1):
        ...
        (pred, truth, psnr_all, ssim_all, all_psnr_mean, all_ssim_mean) = test(epoch, logger)

        if all_psnr_mean > psnr_max:
            psnr_max = all_psnr_mean
            if all_psnr_mean > 29 :
                name = result_path + '/' + 'Test_{}_{:.2f}_{:.3f}'.format(epoch, psnr_max, all_ssim_mean) + '.mat'
